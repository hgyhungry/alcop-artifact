...
@autotvm.register_topi_compute("dense_tensorcore.exp_cuda")
def dense_tensorcore(cfg, data, weight, out_dtype=None):
...
@autotvm.register_topi_schedule("dense_tensorcore.exp_cuda")
def schedule_dense_tensorcore(cfg, outs, exp=True, double_buffer=False):
    """Schedule dense operator using Tensorcore and auto-pipelining for Ampere arch"""
    outs = [outs] if isinstance(outs, te.tensor.Tensor) else outs
    s = te.create_schedule([x.op for x in outs])

    def _callback(op):
        if op.tag == "dense_tensorcore_ldmatrix_cuda":
            _schedule_dense_tensorcore(cfg, s, op.output(0), exp=exp, double_buffer=double_buffer)
    
    traverse_inline(s, outs[0].op, _callback)
    return s

def dense_tensorcore_ldmatrix_cuda(A, B, bias=None, out_dtype=None):
...
def _schedule_dense_tensorcore(cfg, s, C, exp=True, double_buffer=False):
    ...
    cfg.define_knob("warp_col_tiles", [1,2,])
    cfg.define_knob("chunk", [2,4])
    ...
    # if exp:
    #     cfg.define_knob("smem_pipestage", [1,2,3,4,5,6])
    #     cfg.define_knob("regf_pipestage", [1,2,4])
    # cfg.define_knob("block_row_warps", [1,2,4])
    # cfg.define_knob("block_col_warps", [1,2,4])
    # cfg.define_knob("warp_row_tiles", [1,2,4])
